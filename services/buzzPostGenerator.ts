...
function generateThumbnailTikTokPerson(text: string, emotion: EmotionType, postType: PostType): string {
  ...
  const background = matchedBg ? matchedBg[1]
    : emotion === '恋愛不安' || emotion === '孤独' || emotion === '失恋'
    ? '昼下がりの太陽の光がたっぷりと差し込むおしゃれなカフェのテラス席。背景には満開の桜や色鮮やかな花々が咲き誇る明るい庭園。木漏れ日が柔らかく注ぐ（不安な内容でも「順調だった頃の楽しい瞬間」として描きコントラストを演出）'
    : '昼下がりの太陽の光が差し込むおしゃれなカフェのテラス席。満開の花々が咲く明るく華やかな庭園。光の粒子と花びらが舞う明るい演出';
