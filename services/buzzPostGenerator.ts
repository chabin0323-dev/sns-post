...
function generateThumbnailTikTokPerson(text: string, emotion: EmotionType, postType: PostType): string {
  const expressionMap: Record<EmotionType, string> = {
    ...
    '嫉妬': 'ちょっとムッとした表情。口をとがらせて横を向いている',
    '孤独': '窓の外を見ているような、ひとりでいる雰囲気。静かで少し物悲しい表情',
    '依存': '誰かにすがるようなしぐさ。少し心細そうで目が潤んでいる',
    '片思い': 'ぼんやりと微笑みながら夢を見ているような表情。頬に手を当てている',
    '復縁願望': '過去を思い出しているような遠い目。やわらかく切ない表情',
    '失恋': '泣いた後のような赤い目。でも前を向こうとしている複雑な表情',
    '期待': 'キラキラした目で前を見ている表情。少し口角が上がっている',
    '安心感': 'ほっとしたような、やわらかく温かい笑顔。目が細くなっている',
    '自己肯定感': '自信を持った表情。背筋がまっすぐで堂々としている',
    '驚き': '目を少し大きく開けた、驚きと納得が混ざった表情',
  };
  const poseMap: Record<PostType, string> = {
    '共感型': '両手を軽く広げ「そうそう！」というジェスチャー。立った状態で少し前傾み',
    '衝撃型': '手を口元に当てて驚いているポーズ。目を見開いて少し後ずさりしている',
    '切ない型': '膝を抱えて座っているか、壁にもたれかかっているポーズ',
    '恋愛依存型': 'スマホを両手で持ち画面を見つめているポーズ。少し前かがみ',
    '暴露型': '口元に人差し指を当てて「内緒ね」というジェスチャー',
    '心理学型': 'あごに手を当てて考え込んでいるポーズ。少し上を見ている',
    'ストーリー型': '少し遠くを見ながら回想しているような自然なポーズ。横顔気味',
  };

  // 記事内容からシーン・小道具を抽出
  const sceneMap: Record<string, string> = {
    'LINE': 'スマホを手に持ち画面を見ている',
    '返信': 'スマホの画面を見つめて待っている',
    '既読': 'スマホを両手で持ち画面を凝視している',
    'カフェ': 'カフェでコーヒーカップを持ちながら',
    '涙': '目に涙を浮かべながら',
    '窓': '窓の外を眺めながら',
    '夜': '夜の雰囲気の中で、照明が少し暗い',
    '布団': '布団の上でスマホを見ている',
  };
  const sceneKeys = Object.keys(sceneMap).filter(kw => text.includes(kw));
  const sceneDesc = sceneKeys.length > 0 ? sceneMap[sceneKeys[0]] : '';
  ...
  if (sceneDesc) lines.push('・' + sceneDesc);
  lines.push(
    '',
    '■ 構図',
    '・人物は画面の中央に配置',
