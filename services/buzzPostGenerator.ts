...
function generateThumbnailTikTokPerson(text: string, emotion: EmotionType, postType: PostType): string {
  ...
  return [
    ...
    '・' + pose,
    '',
    '■ 構図（CapCut最適化・後からテキストを重ねるための余白設計）',
    '・' + composition,
    '・人物は画面の中央〜下寄りに配置し、上部と下部に人物や物のない余白（背景がそのまま見えるスペース）を確保すること',
    '・背景の花や光の粒子は、余白部分にも自然に配置してよい',
    '',
    '■ 絶対厳守：文字・ぼかしの禁止',
    '・画像内に文字・ロゴ・透かし・タイトル・日本語や英語のテキストは一切描き込まないこと（完全な写真・イラストのみ）',
    '・画像の上部や下部を意図的にぼかしたり、白飛び・グラデーションで覆ったり、不透明度を下げたりしないこと',
    '・画像全体を最初から最後まで鮮明でくっきりとした高解像度で描くこと（余白は「人物や物がない構図」であって「ぼかし加工」ではない）',
    '',
    '■ 追加指示',
    '・背景は太陽の光と鮮やかな花々で画面全体を明るく満たすこと。暗い部分は一切作らないこと',
    '・光の粒子・花びら・小さなハートなどの演出を余白部分に効果的に加えること',
