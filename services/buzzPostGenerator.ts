...
function generateThumbnailTikTokPerson(text: string, emotion: EmotionType, postType: PostType): string {
  // ============================================================
  // 記事内容を総合分析して人物・背景・表情・ポーズを決定
  // 毎回記事に忠実な画像生成指示文を生成する
  // ============================================================

  // ① 記事の冒頭テーマを抽出
  const firstLine = text.split('\n').filter(l => l.trim().length > 5)[0]?.trim().slice(0, 40) ?? '';

  // ② 背景を記事内容から決定（優先度順・最初にマッチしたもの）
  const bgCandidates: [string, string][] = [
    ['夜', '夜の薄暗い部屋。スマホの画面の光だけが照らす雰囲気。窓の外に夜景が見える'],
    ['朝', '朝日が差し込む明るい部屋。清々しく希望を感じる光'],
    ['カフェ', 'おしゃれなカフェの内装。テーブルにコーヒーカップ。温かみのある照明'],
    ['公園', '緑豊かな公園。やわらかい自然光。木漏れ日'],
    ['海', '海辺。水平線が広がる開放的な背景。風を感じる雰囲気'],
    ['雨', '雨の日。窓に水滴。室内から外を見るような雰囲気'],
    ['夕暮れ', '夕暮れの空。オレンジと紫のグラデーション。感傷的な雰囲気'],
    ['学校', '学校の廊下または教室。青春を感じる空間'],
    ['職場', 'オフィス。デスクと窓。都市の景色が背景'],
    ['布団', '寝室。布団やベッドの上。夜の柔らかい照明'],
    ['デート', '夜の街。イルミネーション。ロマンチックな雰囲気'],
    ['初デート', '明るいカフェまたは街中。設定はおしゃれで明るい'],
    ['LINE', '夜の部屋。スマホの画面の青白い光が顔を照らす'],
    ['返信', '夜の部屋。静かで少し緊張感のある雰囲気'],
    ['失恋', '雨の日の窓際。または夕暮れの公園。物悲しい雰囲気'],
    ['復縁', '夕暮れの街。または思い出の場所を想起させる空間'],
    ['告白', '夕暮れの公園または校舎前。ドキドキ感のある空間'],
  ];
  const matchedBg = bgCandidates.find(([kw]) => text.includes(kw));
  const background = matchedBg
    ? matchedBg[1]
    : emotion === '恋愛不安' ? '夜の薄暗い部屋。スマホの光だけが顔を照らす'
    : emotion === '失恋' ? '雨の日の窓際。涙のような雨粒が窓に'
    : emotion === '期待' ? '明るい日差しの中。カフェまたは街中'
    : emotion === '安心感' ? '温かみのある明るい部屋。やわらかい自然光'
    : emotion === '孤独' ? '夜の部屋。一人でいる静かな空間'
    : '室内の柔らかい照明の中。シンプルで落ち着いた空間';

  // ③ ポーズ・シーンを記事内容から決定
  const poseCandidates: [string, string][] = [
    ['LINE', 'スマホを両手で持ち、画面を見つめて返信を待っているポーズ'],
    ['返信', 'スマホを持ちながら、画面を見て少し首をかしげているポーズ'],
    ['既読', 'スマホを見て、少し固まったような表情と姿勢'],
    ['告白', '胸の前で両手を組み、緊張しながら何かを伝えようとするポーズ'],
    ['別れ', 'うつむきながら、指先を見つめているポーズ'],
    ['復縁', '窓の外を見つめながら、過去を思い出しているポーズ'],
    ['涙', '頬に手を当て、涙をこらえているポーズ'],
    ['初デート', '鏡の前で服装を確認しているような、身支度のポーズ'],
    ['失恋', '膝を抱えて座っているか、壁にもたれかかっているポーズ'],
    ['デート', '少し恥ずかしそうに微笑みながら、髪をかき上げるポーズ'],
    ['自信', '背筋を伸ばして前を向き、自信に満ちた立ち姿'],
    ['幸せ', '両手を胸に当てて、幸せをかみしめているポーズ'],
  ];
  const matchedPose = poseCandidates.find(([kw]) => text.includes(kw));
  const pose = matchedPose
    ? matchedPose[1]
    : postType === '共感型' ? '少し前傾みになりながら「そうそう！」と共感するジェスチャー'
    : postType === '衝撃型' ? '手を口元に当てて、驚きを表現するポーズ'
    : postType === '切ない型' ? '膝を抱えてうつむくか、窓の外を見るポーズ'
    : postType === '恋愛依存型' ? 'スマホを胸に抱えるように持ち、待ちわびているポーズ'
    : postType === '暴露型' ? '口元に人差し指を当てた「内緒」のジェスチャー'
    : postType === '心理学型' ? 'あごに手を当てて考え込む、思索のポーズ'
    : '自然体で立ち、カメラを見ているポーズ';

  // ④ 表情を感情から決定
  const expressionMap: Record<EmotionType, string> = {
    '共感': '「わかる！」という共感の笑顔。目を細めてうなずくような表情',
    '恋愛不安': '眉がかすかに寄り、目が少し揺れている不安げな表情',
    '切なさ': '遠くを見つめる切ない横顔。目に涙をこらえている',
    '後悔': '下を向いて考え込み、目を閉じかけている後悔の表情',
    '嫉妬': '口をとがらせて横を向く、少しムッとした表情',
    '孤独': '静かで物悲しい表情。誰かを求めるような目',
    '依存': '心細そうで目が潤んでいる、すがるような表情',
    '片思い': '頬に手を当てて夢見るような、ぼんやりとした笑顔',
    '復縁願望': '過去を思い出すような遠い目。切なくやわらかい表情',
    '失恋': '泣いた後の赤い目。それでも前を向こうとする複雑な表情',
    '期待': '口角がかすかに上がり、キラキラした目で前を見ている',
    '安心感': '目が細くなるほっとした温かい笑顔',
    '自己肯定感': '自信に満ちた堂々とした表情。背筋がまっすぐ',
    '驚き': '目を大きく開けた、驚きと納得が混ざった表情',
  };

  // ⑤ 服装を感情・シーンから決定
  const outfitCandidates: [string, string][] = [
    ['夜', 'ゆったりとした部屋着またはシンプルなルームウェア'],
    ['カフェ', 'カジュアルでおしゃれなコーデ。明るい色のトップス'],
    ['初デート', 'きれいめワンピースまたはおしゃれなカジュアルコーデ'],
    ['デート', 'フェミニンなワンピースまたはきれいめカジュアル'],
    ['失恋', 'シンプルなTシャツとデニム。素朴な普段着'],
    ['職場', 'オフィスカジュアル。清潔感のある服装'],
    ['海', '明るい色のサマーカジュアル。風を感じる軽やかな服'],
  ];
  const matchedOutfit = outfitCandidates.find(([kw]) => text.includes(kw));
  const outfit = matchedOutfit
    ? matchedOutfit[1]
    : emotion === '恋愛不安' ? 'ゆったりとした部屋着またはカジュアルなワンピース'
    : emotion === '期待' ? '明るい色のカジュアルコーデ。少しおしゃれ感あり'
    : emotion === '自己肯定感' ? 'きれいめカジュアル。清潔感があり自信を感じる服装'
    : '清潔感のあるナチュラルなカジュアルスタイル';

  // ⑥ 髪型を記事内容から決定
  const hairStyle = emotion === '失恋' || emotion === '孤独'
    ? 'ダウンヘアまたはゆるくまとめた髪。少し乱れた自然な感じ'
    : emotion === '期待' || emotion === '安心感'
    ? 'ふんわりとしたダウンヘアまたはポニーテール。明るい印象'
    : emotion === '自己肯定感'
    ? 'すっきりとまとめた髪またはきれいなストレートヘア'
    : 'ミディアム〜ロングのゆるふわヘア。自然な黒髪または暗めブラウン';
