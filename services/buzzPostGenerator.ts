...
function generateThumbnailTikTokPerson(text: string, emotion: EmotionType, postType: PostType): string {
  ...
  const sceneKeywords: [string, string][] = [
    ['LINE', 'スマホを手に持ち、LINEの画面を見つめている'],
    ['返信', 'スマホを両手で持ち、メッセージの返信を待っている'],
    ['既読', 'スマホの画面を見て既読スルーに気づいた瞬間の表情'],
    ['涙', '目に涙を浮かべながら、こらえているような表情'],
    ['夜', '夜の薄暗い部屋で、スマホの光だけが顔を照らしている'],
    ['布団', '布団の上でスマホを持ちながら横になっている'],
    ['カフェ', 'カフェでコーヒーカップを前に、ぼんやりとしている'],
    ['窓', '窓の外を眺めながら、物思いにふけっている'],
    ['職場', 'オフィスで少し疲れた表情をしている'],
    ['告白', '緊張した表情で、何かを伝えようとしている'],
    ['別れ', 'うつむきながら、悲しみをこらえている'],
    ['復縁', '過去を思い出すように遠くを見つめている'],
  ];
  const matchedScene = sceneKeywords.find(([kw]) => text.includes(kw));
  const sceneDesc = matchedScene ? matchedScene[1] : '';

  // 感情別・詳細な表情
  const expressionMap: Record<EmotionType, string> = {
    '共感': 'うんうんとうなずくような表情。「わかる！」という共感の笑顔',
    '恋愛不安': 'スマホを見つめながら不安そうな表情。眉がかすかに寄り、目が少し揺れている',
    '切なさ': '遠くを見つめる、少し寂しそうな横顔。目に涙をこらえている',
    '後悔': '下を向いて考え込むような表情。手を胸に当てて、目を閉じかけている',
    '嫉妬': 'ムッとした表情。口をとがらせて横を向いている',
    '孤独': '窓の外を見ているような静かな表情。少し物悲しい',
    '依存': '誰かを求めるような、心細そうな表情。目が潤んでいる',
    '片思い': 'ぼんやりと微笑みながら夢見るような表情。頬に手を当てている',
    '復縁願望': '過去を思い出すような遠い目。切なくやわらかい表情',
    '失恋': '泣いた後のような赤い目。でも前を向こうとしている複雑な表情',
    '期待': 'キラキラした目で前を見ている表情。口角がかすかに上がっている',
  ...
  // 服装を感情に合わせて変化
  const outfitMap: Partial<Record<EmotionType, string>> = {
    '恋愛不安': 'ゆったりとした部屋着またはカジュアルなワンピース',
    '切なさ': 'やわらかいニットやふんわりしたブラウス',
    '失恋': 'シンプルなTシャツとデニム、素朴な普段着',
    '期待': '少しおしゃれなカジュアルコーデ、色は明るめ',
    '自己肯定感': 'きれいめカジュアル、清潔感のある服装',
    '孤独': 'ゆったりとしたシンプルな服、落ち着いた色',
    '共感': 'ナチュラルでかわいらしいカジュアルスタイル',
  };
  const outfit = outfitMap[emotion] ?? '清潔感のあるナチュラルな服装';
  ...
  const lines = [
    ...
    '',
    '■ シーン・ポーズ（記事内容から生成）',
    '・' + (sceneDesc || postType + 'の内容に合った自然なポーズ'),
    '',
    '■ 構図',
    '・人物は画面の中央に配置',
    '・文字は入れない（人物のみ・テキストなし）',
    '・背景はシンプルな単色または淡いグラデーション',
    '',
    '■ 感情トーン：' + emotion,
  ];
  return lines.join('\n');
